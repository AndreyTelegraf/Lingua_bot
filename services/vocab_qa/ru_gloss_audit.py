...
def run_noun_audit(db_path: str, artifacts_dir: str = "artifacts") -> dict:
    ...
    rows = conn.execute("""
        select id, lemma, pos
        from vocab_items
        where is_active=1 and pos='noun'
        order by id
    """).fetchall()

    audit_jsonl = out / "noun_ru_gloss_audit.jsonl"
    review_csv = out / "noun_ru_gloss_review.csv"
    reject_csv = out / "noun_ru_gloss_reject_auto.csv"
    summary_json = out / "noun_ru_gloss_summary.json"
    ...
    with audit_jsonl.open("w", encoding="utf-8") as jf, \
        ...
        for row in rows:
            ...
            """, (row["id"],)).fetchone()
            gloss = correct[0] if correct else ""
            result = evaluate_ru_gloss(lemma=row["lemma"], pos=row["pos"], gloss=gloss)
