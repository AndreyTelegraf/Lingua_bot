...
def run_adverb_audit(db_path: str, artifacts_dir: str = "artifacts") -> dict:
    ...
    rows = conn.execute("""
        select id, lemma, pos
        from vocab_items
        where is_active=1 and pos='adverb'
        order by id
    """).fetchall()

    audit_jsonl = out / "adverb_ru_gloss_audit.jsonl"
    review_csv = out / "adverb_ru_gloss_review.csv"
    reject_csv = out / "adverb_ru_gloss_reject_auto.csv"
    summary_json = out / "adverb_ru_gloss_summary.json"
    ...
    with audit_jsonl.open("w", encoding="utf-8") as jf, \
        ...
        for row in rows:
            ...
            result = evaluate_adverb_ru_gloss(lemma=row["lemma"], pos=row["pos"], gloss=gloss)
