...
def run_adjective_audit(db_path: str, artifacts_dir: str = "artifacts") -> dict:
    ...
    rows = conn.execute("""
        select id, lemma, pos
        from vocab_items
        where is_active=1 and pos='adjective'
        order by id
    """).fetchall()

    audit_jsonl = out / "adjective_ru_gloss_audit.jsonl"
    review_csv = out / "adjective_ru_gloss_review.csv"
    reject_csv = out / "adjective_ru_gloss_reject_auto.csv"
    summary_json = out / "adjective_ru_gloss_summary.json"
    ...
    with audit_jsonl.open("w", encoding="utf-8") as jf, \
        ...
        for row in rows:
            ...
            result = evaluate_adjective_ru_gloss(lemma=row["lemma"], pos=row["pos"], gloss=gloss)
