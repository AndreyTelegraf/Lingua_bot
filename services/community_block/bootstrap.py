...
def ensure_default_chats(conn) -> None:
    allowed_keys = set(DEFAULT_COMMUNITY_CHAT_KEYS)

    for row in DEFAULT_CHAT_DEFINITIONS:
        chat_key = str(row["chat_key"])
        if chat_key not in allowed_keys:
            continue

        existing = repo.get_chat_by_key(conn, chat_key=chat_key)
        if existing is None:
