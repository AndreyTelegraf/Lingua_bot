...
def test_patchable_start_from_vocab_runtime_returns_legacy_when_disabled() -> None:
    conn = _conn()
    try:
        r = patchable_start_from_vocab_runtime(
            conn,
            user_id=1,
            attempt_id=2,
...
def test_patchable_start_from_vocab_runtime_routes_cat_when_enabled() -> None:
    conn = _conn()
    try:
        ...
        assert loaded.metadata["source"] == "patch-test"
    finally:
        conn.close()


def test_patchable_answer_from_vocab_runtime_continues_existing_cat_session() -> None:
    conn = _conn()
    try:
        bank = [_item(10, -0.3), _item(20, 0.2), _item(30, 0.8)]
        started = patchable_start_from_vocab_runtime(
            conn,
            user_id=123,
            attempt_id=778,
            feature_enabled=True,
        ...
        r = patchable_answer_from_vocab_runtime(
            conn,
            user_id=123,
            attempt_id=778,
            feature_enabled=True,
