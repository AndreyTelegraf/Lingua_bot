...
def test_run_verb_audit(tmp_path: Path):
    db = tmp_path / "t.db"
    ...
    conn.execute("""
        ...
        values
        (1, 'achar', 'achar', 'ачар', null, null, 'verb', null, 1, '', '', null, null, null, null, null)
    """)
    vals = ["ачар","дом","стол","кот","река","лес"]
    ...
    summary = run_verb_audit(str(db), str(tmp_path))
