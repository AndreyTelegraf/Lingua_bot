...
def test_handler_api_happy_path() -> None:
    conn = _conn()
    try:
        fsm, view1 = start_command(conn, user_id=42)
        assert view1 is not None
        assert view1['item_id'] == 1
        assert len(view1['keyboard']) == 6
        assert view1['keyboard'][0]['callback_data'] == 'vocab:pick:101'

        fsm, out1 = answer_callback(conn, fsm=fsm, callback_data='vocab:pick:101')
        assert out1['answer_result']['is_correct'] is True
        assert out1['next_view'] is not None
        assert out1['next_view']['item_id'] == 2

        fsm, out2 = answer_callback(conn, fsm=fsm, callback_data='vocab:pick:201')
        assert out2['answer_result']['is_correct'] is False
        assert out2['next_view'] is not None
        assert out2['next_view']['status'] == 'finished'
