...
def test_handler_layer_happy_path() -> None:
    conn = _conn()
    try:
        start = handle_vocab_start(conn=conn, user_id=42)
        assert start['ok'] is True
        assert start['text'] == 'casa'
        assert len(start['keyboard']) == 6
        assert start['finished'] is False

        fsm = start['fsm']

        step1 = handle_vocab_callback(conn=conn, fsm=fsm, callback_data='vocab:pick:101')
        assert step1['ok'] is True
        assert step1['answer_result']['is_correct'] is True
        assert step1['text'] == 'janela'
        assert len(step1['keyboard']) == 6
        assert step1['finished'] is False
