...
def test_flow_finished_state_returns_full_summary() -> None:
    conn = _conn()
    try:
        state = begin_flow(conn, user_id=42)
        state, _ = next_step(conn, state=state)
        state, _ = answer_step(conn, state=state, answer_text="дом")
        state, _ = next_step(conn, state=state)
        state, _ = answer_step(conn, state=state, answer_text="неверно")
        state, _ = next_step(conn, state=state)
