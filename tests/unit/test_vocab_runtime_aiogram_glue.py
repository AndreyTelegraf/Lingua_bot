...
def test_aiogram_glue_happy_path() -> None:
    conn = _conn()
    try:
        start_out = handle_start(conn, user_id=42)
        fsm = start_out['fsm']
        view1 = start_out['view']
        assert view1 is not None
        assert view1['item_id'] == 1

        out1 = handle_callback(conn, fsm=fsm, callback_data='vocab:pick:101')
        fsm = out1['fsm']
        assert out1['answer_result']['is_correct'] is True
        assert out1['next_view']['item_id'] == 2

        out2 = handle_callback(conn, fsm=fsm, callback_data='vocab:pick:201')
