...
IN_CSV = ROOT / "data/master_source_v1/processed/staging_mixed_legacy_length_workbench_wave1/mixed_repair_or_deactivate_legacy_phrase.csv"
OUT_DIR = ROOT / "data/master_source_v1/processed/staging_deactivate_legacy_phrase_wave1"
...
if not targets:
    raise SystemExit("No targets found in mixed_repair_or_deactivate_legacy_phrase.csv")

backup = OUT_DIR / f"lingua_staging.before_deactivate_legacy_phrase_wave1.{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}.db"
...
with (OUT_DIR / "deactivated_items.csv").open("w", encoding="utf-8", newline="") as f:
    cols = [
        ...
        "source_family", "reason_codes", "choices_preview"
    ]
