...
IN_CSV = ROOT / "data/master_source_v1/processed/staging_legacy_manual_review_batch_pack_v1/legacy_manual_review_wave_a_pilot_safe_1k2k.csv"
OUT_DIR = ROOT / "data/master_source_v1/processed/staging_deactivate_wave_a_short_pilot_safe_v1"
...
seen = set()
for r in rows:
    if r.get("operator_hint") != "review_pack_or_deactivate":
        continue
    ...
    targets.append({
        ...
        "pos": r["pos"],
        "bin_name": r["bin_name"],
        "freq_rank": int(r["freq_rank"]) if r.get("freq_rank") else None,
        "source_family": r["source_family"],
        "priority_score": int(r["priority_score"]) if r.get("priority_score") else None,
        "priority_reasons": r["priority_reasons"],
        "choices_preview": r["choices_preview"],
    })

if not targets:
    raise SystemExit("No targets matched operator_hint=review_pack_or_deactivate")

backup = OUT_DIR / f"lingua_staging.before_deactivate_wave_a_short_pilot_safe_v1.{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}.db"
...
for t in targets:
    ...
    before.append({
        **row,
        ...
        "priority_reasons": t["priority_reasons"],
...
for t in targets:
    row = cur.execute(
        f"SELECT {pk_col} AS item_pk, lemma, correct_answer, pos, {active_col} AS active_value "
        f"FROM vocab_items WHERE {pk_col} = ?",
        (t["item_pk"],),
    ).fetchone()
...
summary = {
    ...
    "bin_counts": dict(Counter(t["bin_name"] for t in targets).most_common()),
    "priority_reason_counts": dict(Counter(
        code
        for t in targets
        for code in (t["priority_reasons"].split("|") if t["priority_reasons"] else [])
        if code
    ).most_common()),
...
with (OUT_DIR / "deactivated_items.csv").open("w", encoding="utf-8", newline="") as f:
    cols = [
        ...
        "source_family", "priority_score", "priority_reasons", "choices_preview"
    ]
