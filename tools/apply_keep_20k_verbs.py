...
CSV = BASE / "tools" / "targeted_expansion_probe_20k_verbs_v2_marked.csv"
OUT_DIR = BASE / "artifacts" / f"apply_keep_20k_verbs_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
...
verbs_20k = conn.execute("""
    SELECT COUNT(*)
    FROM vocab_items
    WHERE is_active = 1 AND bin_name = '20K' AND pos = 'verb'
""").fetchone()[0]
...
summary = {
    ...
    "bins_after": bins,
    "active_20k_verbs_after": verbs_20k,
    "backup": str(backup),
    "csv_source": str(CSV),
}
