...
def load_reject_ids(reject_csv: Path) -> list[int]:
...
def count_review_rows(review_csv: Path) -> int:
...
def main() -> int:
    ...
    audit_summary = run_noun_audit(args.db, str(artifacts_dir))
    reject_csv = artifacts_dir / "noun_ru_gloss_reject_auto.csv"
    review_csv = artifacts_dir / "noun_ru_gloss_review.csv"
    ...
    out = artifacts_dir / "noun_semi_auto_pipeline_summary.json"
