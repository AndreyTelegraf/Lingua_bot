...
OUT = ROOT / "artifacts" / f"adverb_followup_apply_v1_{time.strftime('%Y%m%d_%H%M%S')}"
OUT.mkdir(parents=True, exist_ok=True)

ALLOW_NEW = {"tão", "aí", "embora", "inclusive"}
ALLOW_INACTIVE = {"além", "algo", "tanto", "pois"}
...
def find_existing(conn: sqlite3.Connection, lemma: str) -> sqlite3.Row | None:
    return conn.execute(
        ...
        FROM vocab_items
        WHERE pos = 'adverb' AND lower(trim(lemma)) = lower(trim(?))
...
def adverb_choices(lemma: str) -> list[str]:
    packs = {
        ...
        "além": ["там", "тоже", "снова", "точно", "сейчас", "довольно"],
        "algo": ["довольно", "немного", "слегка", "примерно", "редко", "обычно"],
        "tanto": ["так сильно", "очень", "довольно", "слишком", "меньше", "редко"],
        "pois": ["так", "поэтому", "тогда", "однако", "точно", "сейчас"],
    }
    return packs[lemma]
...
def main() -> None:
    ...
    for lemma in sorted(ALLOW_NEW | ALLOW_INACTIVE):
        ...
        row = seed_rows.get(lemma)
        if not row:
            skipped.append({"lemma": lemma, "reason": "missing_seed"})
            continue

        ru_gloss = str(row["ru_gloss"]).strip()
        if not ru_gloss:
            skipped.append({"lemma": lemma, "reason": "missing_ru_gloss"})
            continue
